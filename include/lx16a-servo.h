...
public:
    ...
    void begin(int baud=115200, char const  *port =  "/dev/ttyUSB0") {

        _baud = baud; 

    

        ser.openDevice(port, _baud);
    }
...
public:
	...
	bool calibrate(int32_t currentAngleCentDegrees,int32_t min_angle_cent_deg, int32_t max_angle_cent_deg){
		if(min_angle_cent_deg>=max_angle_cent_deg){
			printf("Min can not be greater than max for  %d halting",(_id));
			while(1);
		}
		int32_t current;
		initialize();
		do{
			...
			if(!isCommandOk()){
				printf("Calibration read FAILED! on index %d ",_id);
		...
		if(min_angle_in_Ticks<0||max_angle_in_Ticks>1000){
			...
			printf("ERROR! bounds of servo ID %d can not be outside hardware limit", _id);
			printf("\tlower %d ticks (Must be > 0)", min_angle_in_Ticks);
			printf("\tcurrent %d ticks  %d deg", currentTicks, pos_read_cached()+staticOffset);
			printf("\tupper %d ticks (Must be < 1000)", max_angle_in_Ticks);
			printf("\terror %d  ticks ", theoretivalMinErrorTicks);
			printf("\tnewset %d ticks ", newSetpointTicks);
			printf("\tnewset deg %d centDeg ", newAngle);
			...
			printf("\nBounds of servo ID %d ok!", _id);
			printf("\tlower %d ticks ", min_angle_in_Ticks);
			printf("\tcurrent %d ticks ", currentTicks);
			printf("\tupper %d ticks ", max_angle_in_Ticks);
		}
		setLimitsTicks(min_angle_in_Ticks,max_angle_in_Ticks);
		minCentDegrees= (min_angle_in_Ticks*24)+staticOffset;
		maxCentDegrees= ((max_angle_in_Ticks)*24)+staticOffset;
		if(abs(min_angle_cent_deg-minCentDegrees)>24)
			printf("FAULT Min angle desired was %d got %d", min_angle_cent_deg, minCentDegrees);
		if(abs(max_angle_cent_deg-maxCentDegrees)>24)
			printf("FAULT max angle desired was %d got %d",max_angle_cent_deg, maxCentDegrees);
		return true;
	}
	void setLimitsTicks(int32_t lower,int32_t upper){
		...
		for(int i=0;i<2;i++){
			...
			if(isCommandOk())
				return;
			if(_bus->_debug)
				printf("Set Limits Failed ID %d Lower %d upper: %d Retry #%d", _id, lower, upper, i);
		}
	}
	...
	void readLimits(){
		...
		do{
			...
			printf(" Min set %d max = %d ", minCentDegrees, maxCentDegrees);
		}while(!isCommandOk());// this is a calibration and can not be allowed to fail
	}
	...
	void move_time(int32_t angle, uint16_t time) {
		...
		if(angle> maxCentDegrees){
			...
			printf("ERROR Capped set at max %d attempted %d", maxCentDegrees, angle);
			angle=maxCentDegrees;
		}
		if(angle<minCentDegrees){
			printf("ERROR Capped set at min %d attempted %d", minCentDegrees, angle);
		...
		printf("Setting ticks %d on ID %d", angle, _id);
	...
	void move_time_and_wait_for_sync(int32_t angle, uint16_t time) {
		...
		if(angle> maxCentDegrees){
			angle=maxCentDegrees;
			printf("ERROR Capped set at max %d", maxCentDegrees);
		}
		if(angle<minCentDegrees){
			angle=minCentDegrees;
			printf("ERROR Capped set at min %d",minCentDegrees);
		}
		if (isMotorMode)
			motor_mode(0);
